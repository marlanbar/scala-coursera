...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Nothing = throw new NoSuchElementException("mostRetweeted of EmptyTweetSet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet =
    ...
    else ((left.union(that)).union(right)).incl(elem)
  ...
  final def mostRetweeted: Tweet =
    ...
    else if (right.isEmpty) (left.incl(elem)).mostRetweeted
...
object GoogleVsApple {
  ...
  def filter_words(s: String, keywords: List[String]): Boolean = {
    for (kw <- keywords) {
      if (s.contains(kw)) return true
    }
    return false
  }
  lazy val googleTweets: TweetSet = (TweetReader.allTweets).filter(tweet => filter_words(tweet.text, google))
  lazy val appleTweets: TweetSet = (TweetReader.allTweets).filter(tweet => filter_words(tweet.text, apple))
